      ...
      {!loading && data && (
        <div className="space-y-6 p-6 pb-12">
          ...
          <div>
            ...
            <p className="text-vm-muted">{data.cleaner.email}</p>
          </div>
          ...
          <section>
            ...
            <div className="rounded-lg border border-vm-border p-4 space-y-2 bg-vm-surface">
              <div>
                <span className="text-xs text-vm-muted">Primary Branch:</span>
              ...
              {data.cleaner.preferredCities && data.cleaner.preferredCities.length > 0 && (
                <div>
                  <span className="text-xs text-vm-muted">Preferred Cities:</span>
            ...
            <div className="rounded-lg border border-vm-border p-4 bg-vm-surface">
              {data.cleaner.availability ? (
                <div className="space-y-3 text-sm">
                  ...
                  <div>
                    <span className="text-vm-muted">Max Daily Jobs:</span>{' '}
                    <span className="font-medium text-vm-text">{data.cleaner.availability.maxDailyJobs}</span>
                  </div>
                  <div>
                    <span className="text-vm-muted">Working Days:</span>
                    <div className="mt-1 flex flex-wrap gap-1">
                      {data.cleaner.availability.workingDays && data.cleaner.availability.workingDays.length > 0 ? (
                        data.cleaner.availability.workingDays.map((day: string, idx: number) => (
                          <span
                            key={idx}
                            className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-gray-200 text-vm-text capitalize"
                          >
                            {day}
                          </span>
                        ))
                      ) : (
                        <span className="text-vm-muted text-xs">None configured</span>
                      )}
                    </div>
                  </div>
                  {data.cleaner.availability.timeRanges &&
                    data.cleaner.availability.timeRanges.length > 0 && (
                      <div>
                        <span className="text-vm-muted">Time Ranges:</span>
                        <div className="mt-1 space-y-1">
                          {data.cleaner.availability.timeRanges.map((range: { start: string; end: string }, idx: number) => (
                            <span
                              key={idx}
                              className="inline-block px-2 py-0.5 rounded text-xs bg-gray-200 text-vm-text mr-1"
                            >
                  ...
                  {data.cleaner.availability.blackoutDates &&
                    data.cleaner.availability.blackoutDates.length > 0 && (
                      <div>
                        <span className="text-vm-muted">Blackout Dates:</span>
                ...
                <p className="text-vm-muted text-sm">No availability configured</p>
              )}
            </div>
          </section>
              ...
              {data.cleaner.trainingStatus ? (
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-vm-muted">Status:</span>
                ...
                <p className="text-vm-muted text-sm">No training status</p>
              )}
            </div>
          </section>
                  ...
                  <div className="space-y-3">
                    {data.ratings.recent.map((rating) => (
                      <div key={rating.id} className="border rounded p-3 bg-white">
                        ...
                        {rating.comment && (
                          <p className="text-sm text-vm-text mt-2">{rating.comment}</p>
                        )}
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-sm text-vm-muted">No reviews yet</p>
                )}
              </div>
            </section>
          )}
          ...
          <section>
            ...
            <div className="rounded-lg border border-vm-border p-4 bg-vm-surface">
              <div className="grid grid-cols-2 gap-4 text-sm mb-4">
                <div>
                  <div className="flex items-center gap-1 text-vm-muted mb-1">
                    <Calendar className="w-4 h-4" />
                    <span>This Week</span>
                  </div>
                  <p className="text-lg font-semibold text-vm-text">{data.stats.weeklyJobs}</p>
                </div>
                <div>
                  <div className="flex items-center gap-1 text-vm-muted mb-1">
                    <Calendar className="w-4 h-4" />
                  ...
                  <p className="text-lg font-semibold text-vm-text">{data.stats.monthlyJobs}</p>
                </div>
                <div>
                  <div className="flex items-center gap-1 text-vm-muted mb-1">
              ...
              {data.performance?.productivityScore !== undefined && (
                <div className="pt-4 border-t border-vm-border">
                  <div className="flex items-center gap-1 text-vm-muted mb-1">
                    <TrendingUp className="w-4 h-4" />
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 bg-gray-200 rounded-full h-2">
                      <div
                        className="bg-vm-navy h-2 rounded-full transition-all"
            ...
            <section>
              ...
              <div className="rounded-lg border border-vm-border p-4 bg-vm-surface">
                ...
                {data.payouts.latest.length > 0 ? (
                  <div className="space-y-2">
                    <p className="text-xs font-medium text-vm-text mb-2">Latest Payouts:</p>
                      ...
                      <div key={payout.id} className="border rounded p-3 bg-white">
                        <div className="flex items-start justify-between mb-1">
                          <div className="flex-1">
                            ...
                            {payout.branch && (
                              <p className="text-xs text-vm-muted mt-0.5">{payout.branch.name}</p>
                            )}
                          </div>
                  ...
                  <p className="text-sm text-vm-muted">No payouts yet</p>
                )}
              </div>
            </section>
          )}
            ...
            <section>
              ...
              <div className="rounded-lg border border-vm-border p-4 bg-vm-surface">
                ...
                {data.compliance.issues.length > 0 && (
                  <ul className="space-y-1 text-sm text-vm-text">
                ...
                {data.compliance.issues.length === 0 && (
                  <p className="text-sm text-vm-muted">All compliance requirements met</p>
                )}
              </div>
            </section>
          )}
          ...
          <section>
            ...
            <div className="rounded-lg border border-vm-border p-4 bg-vm-surface">
              {data.upcomingJobs.length === 0 ? (
                <p className="text-sm text-vm-muted">No upcoming jobs</p>
              ) : (
                <div className="space-y-2">
                  {data.upcomingJobs.map((job) => (
                    <div key={job.id} className="border rounded p-3 bg-white">
                      <div className="flex items-start justify-between mb-1">
                        <div className="flex-1">
                          <p className="font-medium text-sm text-vm-text">
                            {job.preferredDate
                          ...
                          {job.preferredTime && (
                            <p className="text-xs text-vm-muted mt-0.5">{job.preferredTime}</p>
                          )}
                        </div>
                      ...
                      {job.address && (
                        <p className="text-xs text-vm-muted mt-1 truncate">{job.address}</p>
                      )}
                    </div>
                  ))}
                </div>
          ...
          <section>
            ...
            <div className="rounded-lg border border-vm-border p-4 bg-vm-surface">
              {data.recentJobs.length === 0 ? (
                <p className="text-sm text-vm-muted">No completed jobs yet</p>
              ) : (
                <div className="space-y-2">
