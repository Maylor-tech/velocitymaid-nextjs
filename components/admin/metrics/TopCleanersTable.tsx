...
export default function TopCleanersTable({ cleaners }: TopCleanersTableProps) {
  if (cleaners.length === 0) {
    return (
      <div className="text-center py-12 text-vm-muted">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {cleaners.map((cleaner) => (
            <tr key={cleaner.cleanerId} className="hover:bg-gray-50">
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-sm text-vm-muted">{cleaner.email}</div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-sm text-vm-text">{cleaner.jobsCompleted}</div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                {cleaner.averageRating !== null ? (
                  ...
                  <span className="text-sm text-vm-muted">—</span>
                )}
              </td>
            </tr>
          ))}
