...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, phone, email, propertyName, villaApplicationId } = body;

    if (!name || !phone || !propertyName) {
      return NextResponse.json(
        { success: false, error: 'Name, phone, and property name are required' },
        { status: 400 }
      );
    }

    // Generate HTML for PDF
    const html = generateContractHTML(villaPartnershipTemplate, {
      name,
      phone,
      email: email || '',
      propertyName,
    ...
    const contractId = `villa-${Date.now()}`;
    ...
    await prisma.contract.create({
      data: {
        ...
        metadata: {
          propertyName,
          villaApplicationId: villaApplicationId || null,
        },
      },
    });
    ...
    console.error('Generate villa contract error:', error);
    return NextResponse.json(
...
<body>
  ...
  <div style="margin-bottom: 30px; padding: 15px; background: #F3F1EB; border-radius: 8px;">
    <p style="margin: 5px 0; font-size: 14px;"><strong>Partner Name:</strong> ${variables.name}</p>
    <p style="margin: 5px 0; font-size: 14px;"><strong>Property Name:</strong> ${variables.propertyName}</p>
