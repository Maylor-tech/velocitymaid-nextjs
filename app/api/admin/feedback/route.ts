...
import { parseGuestClassFromAdminNotes } from '@/lib/feedback/guestFeedbackClassification';
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      items: rows.map((r) => {
        const ops = parseGuestClassFromAdminNotes(r.adminNotes);
        return {
          id: r.id,
          source: r.source,
          status: r.status,
